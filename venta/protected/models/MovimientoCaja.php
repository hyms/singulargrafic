...
class MovimientoCaja extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('idCaja, fecha, monto, obs, tipo, idVenta, idComprovante, idEmpleado', 'required'),
			array('idCaja, monto, tipo, idVenta, idComprovante, idEmpleado', 'numerical', 'integerOnly'=>true),
			array('obs', 'length', 'max'=>200),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, idCaja, fecha, monto, obs, tipo, idVenta, idComprovante, idEmpleado', 'safe', 'on'=>'search'),
		);
	}
