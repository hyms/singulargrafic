	...
	$this->widget('zii.widgets.grid.CGridView', array(
		...
		'columns'=>array(
			...
			array(
					...
					'filter'=>CHtml::activeDropDownList($productos,'material',CHtml::listData(Producto::model()->findAll(array('group'=>'material','select'=>'material')),'material','material'),array("class"=>"form-control input-sm",'prompt'=>'')),
			),
			array(
					'header'=>'Color',
					'value'=>'$data->idProducto0->color',
					'filter'=>CHtml::activeDropDownList($productos,'color',CHtml::listData(Producto::model()->findAll(array('group'=>'color','select'=>'color')),'color','color'),array("class"=>"form-control input-sm",'prompt'=>'')),
			),
			array(
					'header'=>'Detalle',
			...
			array(
					...
					'filter'=>CHtml::activeDropDownList($productos,'industria',CHtml::listData(Producto::model()->findAll(array('group'=>'industria','select'=>'industria')),'industria','industria'),array("class"=>"form-control input-sm",'prompt'=>'')),
			),
			array(
					'header'=>'Cant.xPaqt.',
					'value'=>'$data->idProducto0->cantXPaquete',
					'filter'=>CHtml::activeDropDownList($productos,'paquete',CHtml::listData(Producto::model()->findAll(array('group'=>'cantXPaquete','select'=>'cantXPaquete')),'cantXPaquete','cantXPaquete'),array("class"=>"form-control input-sm",'prompt'=>'')),
			),
			array(
					'header'=>'Precio CF',
			...
			array(
					'header'=>'',
					'type'=>'raw',
					'value'=>'CHtml::link("Añadir","#",array("onclick"=>\'newRow("\'.$data->idAlmacenProducto.\'");\'))',
			),
		)
	));
?>
