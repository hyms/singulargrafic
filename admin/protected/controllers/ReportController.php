...
class ReportController extends Controller
{
    ...
    protected function getSaldoAnterior($idAlmacen)
    {
        $startDateS=date("Y")."-".(date("m")-1)."-1 00:00:00";
        $endDateS=date("Y")."-".date("m")."-1 00:00:00";
        $saldos = SaldoProducto::model()->with('idAlmacen0')->with('idAlmacen0.idProducto0')->findAll(array('condition'=>'`idAlmacen0`.idAlmacen='.$idAlmacen.' and `t`.fechaSaldo Between "'.$startDateS.'" and "'.$endDateS.'"','order'=>'idProducto0.Material,idProducto0.codigo, idProducto0.detalle'));

        if(empty($saldos) || count($saldos)==0)
        {
            $almacenes = AlmacenProducto::model()->findAll('idAlmacen='.$idAlmacen);
            foreach ($almacenes as $key => $almacen)
            {
